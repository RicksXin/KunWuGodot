...
| 数据 | 未来权威方 | Demo 当前实现 |
|---|---|---|
| 账号、档案版本 | 服务端 | `Game` autoload + `user://kunwu_profile.json` |
| Wallet、生产、消费 | 服务端 | `Game` 领域方法与本地存档 |
| 修士、队伍、灵息、装载 | 服务端 | `Game` 中的本地远征状态 |
| 地图位置、迷雾、固定对象 | 服务端会话 | `Game.expedition` 本地快照 |
| 战斗指令与结算 | 服务端结算器 | GDScript `KWCombatResolver` |
| 战利品与回城入账 | 服务端原子事务 | `Game` 的归营结算与原子文件替换 |
| 页面开关、动画、音效 | 客户端 | Godot 表现层 |
...
事件。D0 由 `res://scripts/domain/combat_resolver.gd` 的 `KWCombatResolver` 直接结算；未来
联网时，远端结算结果也必须转换为相同 `CombatEvent` 语义后再交给表现层。
...
- DTO 数值保持有符号 64 位整数范围；若未来 Web 客户端需要互操作，再在 HTTP 边界收窄。
