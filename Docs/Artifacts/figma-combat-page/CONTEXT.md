...
- 最新位置调整（2026-09-05）：按用户最新要求，我方队伍从 `y=592` 整体上移 20px，卡片起点现为 `y=572`，下文阶段记录中的 `y=492` 已废弃。人物、绿色调试底和信息层一起移动，卡片内坐标保持原值；底部遮罩起点随卡片下沿同步到 `y=777`，避免覆盖姓名和行动条。技能条位置保持 `(117.5,491)`。本次专项回归输出 `ANIMATED_PORTRAIT_VALIDATION_OK`，待实机视觉复核。
