...
> 2026-08-24 计费校准：Map01 高清山体对照任务使用 `nano-banana-run`、`1K`、
> `1:1`、`normal` 与两张参考图。提交前官方 API 文档标价和用户批准上限均为
> 10 点，服务端任务结果却返回 `credit_cost=14`，账户余额从 2438 降为 2424，
> 实际超出批准上限 4 点。因此新的 `nano-banana-run` 付费提交暂停；在官方价格
> 能覆盖参考图附加成本并可稳定计算最高积分前，不得通过提高人工估算、改用网页
> 或绕过门禁继续提交。已完成 Job 只允许免费轮询、下载和本地处理。
