...
- 用户已确认“配套高清底图 + 外部人工标注 JSON + Godot 接入”的同类地图流程。收到此类地图时，先阅读
  `Docs/Tech/高清地图与外部标注接入规范.md`，默认复用已验证方法；不自动迁移既有正式 Map01。
- 底图与 JSON 必须来自同批导出；区域直接读取原始坐标，显示、实际移动及寻路共享同一变换和判定。
  不凭图片重画、不过滤大区域、不保留旧手绘路线限制，也不把 Overlay 对齐当成实际碰撞已接入。
- 后续地图参数从各自清单读取，不能照抄本次尺寸、区域数量或缩放值。图层语义与验收按上述规范执行。
