...
func _ready() -> void:
	...
	var help := Label.new()
	help.text = "左侧：实时3D模型　|　右侧：营地对照。修士沿石阶进出，侧亭候招、持册接待；幡旗与灯笼同步暂停。"
	add_child(help)
	...
	building_visual = load("res://resources/prototypes/camp_recruit_3d/recruit.glb").instantiate()
	...
	banners = load("res://scripts/prototypes/camp_recruit_lanterns.gd").new()
	banners.name = "LanternMotion"
	...
	camp.buildings.get_node("recruit").hide()
	camp.actor.hide()
	camp.overlay.hide()
	...
	billboard.scale = Vector2.ONE*0.32
	billboard.position = camp.buildings.get_node("recruit").position
	...
	if OS.get_cmdline_user_args().has("--capture-lanterns"): call_deferred("capture_lanterns")
func fit_map() -> void:
...
func capture_views() -> void:
	for value in [0,90,180,270]:
		...
		get_viewport().get_texture().get_image().save_png("res://Docs/Artifacts/camp-tilemap-exploration/recruit-3d-%d.png" % value)
		assert(viewport.get_texture().get_image().save_png("res://art/candidates/camp-recruit-3d-v2/preview-%d.png" % value) == OK)
	print("PASS recruit 3D: loaded GLB; yaw 0/90/180/270 captured; upright model with fixed camera")
	get_tree().quit()


func capture_lanterns() -> void:
	set_yaw(0)
	var directory := "res://art/candidates/camp-recruit-3d-v2/frames"
	...
	for frame in range(192):
		...
		if frame == 0:
			get_viewport().get_texture().get_image().save_png("res://Docs/Artifacts/camp-tilemap-exploration/recruit-lanterns-ingame.png")
	var detail_directory := "res://art/candidates/camp-recruit-3d-v2/detail-frames"
	DirAccess.make_dir_recursive_absolute(detail_directory)
	camera.size = 5.7
	camera.position = Vector3(5.0, 3.7, 10.0)
	camera.look_at(Vector3(0, 1.00, 1.25))
	for frame in range(192):
		banners.seek_preview(float(frame) / 8.0)
		await get_tree().process_frame
		RenderingServer.force_draw(false)
		assert(viewport.get_texture().get_image().save_png("%s/frame-%03d.png" % [detail_directory, frame]) == OK)
	print("PASS recruit: 192 whole-building + 192 visitor-detail frames at 8 FPS")
	get_tree().quit()
