...
func _run() -> void:
	...
	assert(packed != null, "Missing difficult brush scene: %s" % SCENE_PATH)
	...
	assert(world != null, "Difficult TileMapDual node was not created")
	assert(world.get_script().resource_path == "res://addons/TileMapDual/tile_map_dual.gd")
	assert(world.get_child_count() == 1, "Difficult TileMapDual display was not created")
	...
	for pattern_name: String in PATTERNS:
		...
		print("difficult_", pattern_name, " masks=", masks)
	...
	for mask in range(1, 16):
		if mask not in seen_masks:
			missing.append(mask)
	assert(missing.is_empty(), "Difficult masks not exercised: %s" % [missing])
	print("Map01 difficult TileMapDual validation OK; masks 0x1-0xF exercised")
	instance.queue_free()
