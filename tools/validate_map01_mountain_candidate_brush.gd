...
func _run() -> void:
	...
	assert(packed != null, "Missing mountain candidate scene")
	var instance := packed.instantiate()
	root.add_child(instance)
	await process_frame
	await process_frame

	var world := instance.get_node("MountainCandidate") as TileMapLayer
	assert(world != null, "MountainCandidate node was not created")
	...
	for pattern_name: String in PATTERNS:
		...
		print("mountain_", pattern_name, " masks=", masks)
	...
	for mask in range(1, 16):
		if mask not in seen_masks:
			missing.append(mask)
	assert(missing.is_empty(), "Mountain masks not exercised: %s" % [missing])
	print("MAP01_MOUNTAIN_CANDIDATE_VALIDATION_OK masks=0x1-0xF")
	instance.queue_free()
